...
(function() {
    ...
    function _revertArgs(key, params) {
        ...
        if (key == "share") {
            ...
            if (desc_ele)
                desc = desc_ele.content;
            if (rel_icon_ele)
                icon = rel_icon_ele.href;
            if (apple_touch_icon_ele)
                icon = apple_touch_icon_ele.href;
    ...
    function _call(func, params, callback, sdk_version) {
        ...
        //alert(JSON.stringify(_msg_string));
    ...
    //对外暴露toutiao, 隐藏ToutiaoJSBridge
    window.toutiao = {
        on : _on,
        trigger : _trigger
    };

    function _initToutiao() {
        var latent_keys = {
        ...
        public_fn = ["appInfo", "login", "comment", "close", "isVisible", "isLogin","playVideo"],
        protected_fn = ["isAppInstalled", "open", "share", "systemShare", "pay", "pageStateChange", "downloadApp", "openThirdApp", "uploadImage", "addChannel", "gallery","copyToClipboard","openCocosPlay"],
        private_fn = [],
        all_fn = public_fn.concat(protected_fn),
        sdk_match = navigator.userAgent.match(/JSSDK\/([\d.]+)/i),
        sdk_version = sdk_match ? sdk_match[1] : 1;


        if (sdk_match) {
            all_fn = all_fn.concat(["config"]);
        } else {
            //旧版app, 不支持config.直接返回支持列表
            toutiao['config'] = function(arg) {
                var callback_success = getParams(arg, "success");
                if (callback_success) {
        ...
        function getParams(arg, key) {
            return key == "params" ? arg : arg[key];
        };

        all_fn.forEach(function(key) {

            toutiao[key] = function(arg) {
                arg = arg || {};
                var params = getParams(arg, "params"),
                callback_error = getParams(arg, "error"),
                callback_success = getParams(arg, "success"),
                callback_fail = getParams(arg, "fail");

                //params['clientID'] ="xxxx";
                key = latent_keys[key] || key;
                _call(key, params, function(o) {
                    o = o || {};
                    ...
                    if (isEmptyObject(o)) {
                        ...
                        //返回值不为空,且忘了传code,认为成功
                        code = 1
                    };

                    if (code == - 1 && callback_error)
                        callback_error(o);
                    if (code == 0 && callback_fail)
                        callback_fail(o);
                    if (code == 1 && callback_success)
                        callback_success(o);
                }, sdk_version);
            }
        });
